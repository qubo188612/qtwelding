...
void keyplcDlg::setbutton(int name)
{
    if(name==0)
    {
        ui->PLCoutputBtn->setText(QString::fromLocal8Bit("插入PLC输出指令"));
        ui->PLCinputBtn->setText(QString::fromLocal8Bit("插入等待PLC输入指令"));
    }
    else
    {
        ui->PLCoutputBtn->setText(QString::fromLocal8Bit("替换PLC输出指令"));
        ui->PLCinputBtn->setText(QString::fromLocal8Bit("替换等待PLC输入指令"));
    }
}
...
void keyplcDlg::on_PLCoutputBtn_clicked()
{
    ...
    {
        ui->record->append(QString::fromLocal8Bit("请填写寄存器地址"));
        return;
    }
    else
    {
        if(b_add==false)
        {
            ui->record->append(QString::fromLocal8Bit("寄存器地址格式错误"));
            return;
        }
    }
    if(ui->plc_data->text().isEmpty())
    {
        ui->record->append(QString::fromLocal8Bit("请填写寄存器内容"));
        return;
    }
    else
    {
        if(b_data==false)
        {
            ui->record->append(QString::fromLocal8Bit("寄存器内容格式错误"));
            return;
        }
        else
        {
            ...
            {
                ui->record->append(QString::fromLocal8Bit("寄存器内容取值是-32768到32767之间"));
                return;
            }
        }
    }
    msg=cmd.cmd_plcwrite(add,data);
    ui->record->append(QString::fromLocal8Bit("插入PLC输出指令成功"));
    cmd_msg=msg;
    done(1);
}
...
void keyplcDlg::on_PLCinputBtn_clicked()
{
    ...
    ui->record->append(QString::fromLocal8Bit("插入等待PLC输入指令成功"));
    cmd_msg=msg;
    done(1);
}
